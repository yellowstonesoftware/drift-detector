...
struct DriftDetector: AsyncParsableCommand {
    ...
    @Option(
        name: .long,
        help: "Kubernetes contexts with aliases in format 'context=alias'. Can be specified multiple times."
    )
    var context: [String] = []
    ...
    @Option(
        name: .long,
        help: "GitHub Personal Access Token for API authentication (if not provided, will use GITHUB_TOKEN)"
    )
    ...
    func validate() throws {
        ...
        guard let token = githubToken ?? ProcessInfo.processInfo.environment["GITHUB_TOKEN"], !token.isEmpty else {
            throw ValidationError("GitHub token must be provided via --github_token or GITHUB_TOKEN environment variable")
        }
    }
